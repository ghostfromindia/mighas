...
class ApiController extends Controller
{
    ...
    public function brach_data(){$menu = MenuItem::where('parent_id',0)->get();
       foreach ($menu as $obj){
    ...
    public function category_meta_auto(){ return 1;
        $categories = Category::all();
        foreach ($categories as $obj){
            $obj->browser_title = $obj->category_name.' | Hykon India Limited';
            $obj->meta_keywords = $obj->category_name.', Hykon';
            $obj->meta_description = $obj->category_name.' | Hykon India Limited';
            $obj->save();
        }

        $categories = Products::all();
        foreach ($categories as $obj){
            $obj->browser_title = $obj->product_name.' | Hykon India Limited';
            $obj->meta_keywords = $obj->product_name.', Hykon';
            $obj->meta_description = $obj->product_name.' | Hykon India Limited';
            $obj->save();
        }
    ...
    public function menu_domain_change(){

        $menu = MenuItem::all();
        foreach ($menu as $obj){
            $obj->url = str_replace("http:", "https:", $obj->url);
            $obj->save();
        }
    }

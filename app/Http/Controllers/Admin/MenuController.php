...
use App\Models\Menu, Input, View, Redirect, DB, Datatables;
...
class MenuController extends BaseController
{
    ...
    public function create()
    {
        ...
        $brands = $this->displayBrandHierarchy();
        return view($this->views . '.form', ['obj'=>$this->model, 'pages'=> $pages, 'categories'=>$categories, 'front_pages'=>$front_pages, 'brands'=>$brands]);
    }
    ...
    public function edit($id) {
        ...
        if($obj = $this->model->find($id)){
            ...
            return view($this->views . '.form', ['obj'=>$obj, 'pages'=> $pages, 'categories'=>$categories, 'front_pages'=>$front_pages, 'brands'=>$brands]);
        } else {
        ...
        if($menu_settings)
        {
            foreach ($menu_settings as $key => $setting) {
                ...
                if(isset($menu[$id]))
                {
                    ...
                    {
                    	$obj->linkable_type = "App\Models\Page";                     
                    ...
                    {
                        $obj->linkable_type = "App\Models\FrontendPage";                     
                        $obj->linkable_id = $menu[$id]['id'];
                    }
                    elseif($item_array[0] == 'category_link')
                    {
                        $obj->linkable_type = "App\Models\Category";                     
                        $obj->linkable_id = $menu[$id]['id'];
                    }
                    elseif($item_array[0] == 'brand_link')
                    {
                        $obj->linkable_type = "App\Models\Brand";                     
                        $obj->linkable_id = $menu[$id]['id'];
                    }
    ...
    public function parse_hierarchical_category_list($array, $currentParent = 0, $selected, $deep=0, $outType='options', $type=null, $not_in=null, $not_type=null)
    {
    	...
    	if($array)
	    	foreach ($array as $key => $item) {
	    		if($item->parent_category_id == $currentParent){
					...
					if($outType == 'options')
						$html .= "<option value='".$item->id."' ".$cur_select.">".$indent.$item->category_name."</option>";
					else
						$html .= '<div class="checkbox">'.$indent.'<input type="checkbox" '.$cur_select.' name="category_links[]" class="category_links" value="category_link-'.$item->id.'" data-id="'.$item->id.'" data-name="'.$item->category_name.'" id="category_links-'.$item->id.'"> <label for="category_links-'.$item->id.'">'.$item->category_name.'</label></div>';
    ...
    public function displayBrandHierarchy()
    {
        ...
        {
            foreach ($options as $key => $item) {
                $html .= '<div class="checkbox"><input type="checkbox"  name="brand_links[]" class="brand_links" value="brand_links-'.$item->id.'" data-id="'.$item->id.'" data-name="'.$item->brand_name.'" id="brand_links-'.$item->id.'"> <label for="brand_links-'.$item->id.'">'.$item->brand_name.'</label></div>';
            }
        }
        return $html;
    }

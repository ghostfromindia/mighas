...
class UserController extends BaseController
{
    ...
    protected function getCollection() {
        $collection = DB::table('users')->select('users.id', 'users.email', DB::raw("CONCAT(users.first_name, ' ', users.last_name) AS name"), 'users.banned_at', 'users.created_at', 'users.updated_at', DB::raw("CONCAT('+',users.country_code, ' ', users.phone_number) AS phone"))->join('role_users', 'users.id', '=', 'role_users.user_id')->where('role_users.role_id', '!=', 2)->whereNull('deleted_at');

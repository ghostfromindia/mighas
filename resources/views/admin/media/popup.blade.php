	   ...
	   <div class="panel with-nav-tabs panel-default">
                ...
                <div class="panel-body">
                    <div class="tab-content">
                        <input type="hidden" id="popupType"  @if($popup_type == 'banner' || $popup_type == 'mobile_banner') value="single_image" @else value="{{$popup_type}}" @endif>
                            ...
                            <div class="upload-wrapper">
                              ...
                              <div id="dropzone" class="dropzone-wrapper">
                                      ...
                                      <span class="button btn-blue input-file">
                                          Browse Files <input type="file" id="mediafileupload" name="files[]" data-url="{{$data_url}}" multiple />
                                      </span>
                              </div>
                        ...
                        <div class="tab-pane @if(count($files)>0) active show @endif" id="tab2Media">
                          ...
                          <div class="row media-list-modal" id="mediaList">
                            @include('admin.media.ajax_index_popup', ['files'=>$files, 'holder_attr'=>$holder_attr])
                          </div>
                          <div class="text-right">
                              ...
                              @else
                                <button class="btn btn-primary" id="addPhotos" @if($popup_type == 'photos') data-url="{{url('admin/photos/save')}}" @elseif($popup_type == 'sliders') data-url="{{url('admin/sliders/update', ['id'=>encrypt($id)])}}" @elseif($popup_type == 'banners') data-url="{{url('admin/banners/update', ['id'=>encrypt($id)])}}" @endif><i class="glyphicon glyphicon-plus-sign"></i> Add Photos</button>
                              @endif
                          </div>
                        </div>
                    </div>
